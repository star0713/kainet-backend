...
//   );
  getBitqueryData(
    `{ ethereum(network: ${chain}) { dexTrades( options: {desc: [\"tradeAmount\", \"trades\"], limit: 30} time: {since: \"2022-09-16T06:16:18.416Z\"} ) { token: smartContract { address { address } } exchange { fullName } trades: count tradeAmount(in: USD) any(of: price) baseCurrency { address symbol name } } } }`,
    "{}"
  );
